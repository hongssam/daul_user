		...
		<div class="row">
			<div class="col-lg-10 offset-lg-1">
				<div class="board-detail-table">
					...
					<input type="hidden" id="notice_idx" value=${contestNoticeVo.notice_idx }>
						<table class="table">
							<thead class="thead-light">
								<tr class="hidden-sm-down">
									<th scope="col" class="text-left">${contestNoticeVo.title}</th>
									<th scope="col" class="text-right board-info">${contestNoticeVo.name} | ${contestNoticeVo.create_date} | 조회 ${contestNoticeVo.view_count}</th>
								</tr>
								<tr class="hidden-sm-up">
									<th scope="col" class="text-left">
										${contestNoticeVo.title}
										<p class="small">${contestNoticeVo.name} | ${contestNoticeVo.create_date} | 조회 ${contestNoticeVo.view_count}</p>
									</th>
								</tr>
							</thead>
							<tbody>
...
<script type="text/javascript">
	...
	function getBeforeNotice() {
		...
		var request = $.ajax({
			url: "/contest/getBeforeNotice.do?notice_idx="+idx,
			method: "get",
		});
	...
	function getAfterNotice() {
		var request = $.ajax({
			url: "/contest/getAfterNotice.do?notice_idx="+idx,
			method: "get",
		});
	...
	function drawBeforeNotice(data){
		...
		if(typeof before_notice_title == "undefined" || before_notice_title == null || before_notice_title == ''){
			...
			str = 	'<a href="/contest/contestNoticeDetail.do?notice_idx='+before_notice_idx+'">'
			+	'<span class="fa-caret-up mr10">&nbsp;&nbsp;&nbsp;윗글</span>'
			+	before_notice_title
			+'</a>'
			+'<hr>';
	...
	function drawAfterNotice(data){
		...
		if(typeof after_notice_title == "undefined" || after_notice_title == null || after_notice_title == ''){
			...
			str = 	'<a href="/contest/contestNoticeDetail.do?notice_idx='+after_notice_idx+'">' 
			+	'<span class="fa-caret-down mr10">&nbsp;&nbsp;&nbsp;아랫글</span>'
			+	after_notice_title
			+'</a>'
			+'<hr>';
	...
	$(document).ready(function(){
  	...
  	function fn_downloadFile(obj){
		...
		location.href = "${pageContext.request.contextPath}/contest/downloadContestNoticeFile.do?save_file_name=" + save_file_name;
  	}
